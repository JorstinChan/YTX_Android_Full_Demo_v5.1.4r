...
public class IMessageSqlManager extends AbstractSQLManager {
    ...
    public static Cursor queryIMessageCursor(long threadId , int limit) {
        ...
        String sql = "SELECT * FROM " + DatabaseHelper.TABLES_NAME_IM_MESSAGE + " WHERE " + IMessageColumn.OWN_THREAD_ID + "= " + threadId + " ORDER BY " + IMessageColumn.RECEIVE_DATE + " ASC LIMIT " + limit + " offset "
                + "(SELECT count(*) FROM " + DatabaseHelper.TABLES_NAME_IM_MESSAGE + " WHERE " + IMessageColumn.OWN_THREAD_ID + "= " + threadId + " ) -" + limit;
        LogUtil.d(TAG, "getCursor threadId:" + threadId + " limit:" + limit + " [" + sql + "]");
